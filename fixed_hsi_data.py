    ...
    try:
        ...
        if ret == RET_OK:
            ...
            if ret == RET_OK:
                ...
                print(f"   數據範圍: {kline_data['time_key'][0]} 到 {kline_data['time_key'][-1]}")
                ...
                for i in range(min(5, len(history_data))):
                    ...
                    change = day['close'] - history_data[-(i+2)]['close'] if i < len(history_data)-1 else 0
                    change_pct = (change / history_data[-(i+2)]['close'] * 100) if i < len(history_data)-1 and history_data[-(i+2)]['close'] != 0 else 0
                    print(f"   {day['date']}: 開{day['open']:.2f} 高{day['high']:.2f} 低{day['low']:.2f} 收{day['close']:.2f} ({change:+.2f}, {change_pct:+.2f}%) 量{day['volume']:,}")
