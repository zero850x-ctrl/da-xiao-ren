#!/usr/bin/env python3
"""
OANDA黃金自動交易系統 - 在Mac上直接運行
使用OANDA API替代MT5，完全兼容macOS
"""
...
class OANDAGoldTrader:
    def __init__(self, use_demo=True):
        ...
        self.symbol = "XAU_USD"  # OANDA的黃金符號
    ...
    def execute_trade(self, signal):
        ...
        if self.today_trades >= self.max_daily_trades:
            self.logger.warning(f"今日已達最大交易次數: {self.today_trades}/{self.max_daily_trades}")
            return False
        
        try:
            lot_size = str(self.config['lot_size'])  # OANDA需要字符串
            
            # 準備訂單數據
            if signal['type'] == 'BUY':
                order_data = {
                    "order": {
                        "units": lot_size,  # 正數表示買入
                        "instrument": self.symbol,
                        "timeInForce": "FOK",  # 立即成交否則取消
                        "type": "MARKET",
                        "stopLossOnFill": {
                            "price": str(round(signal['stop_loss'], 2)),
                            "timeInForce": "GTC"
                        },
                        "takeProfitOnFill": {
            ...
            # 發送訂單
            r = orders.OrderCreate(self.account