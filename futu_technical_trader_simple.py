...
class SimpleTechnicalTrader:
    ...
    def analyze_stock(self, code):
        ...
        if ret != RET_OK or len(hist_data) < 20:
            return None
        ...
        analysis = {
            ...
            'price_history': prices.tolist()[-20:]  # 最近20个价格
        }
    ...
    def generate_trading_signal(self, analysis):
        ...
        # MACD信号
        macd, signal_line, histogram = analysis['macd']
        ...
        if len(price_history) >= 5:
            recent_change = (price_history[-1] - price_history[-5]) / price_history[-5] * 100
            if recent_change > 2:  # 近期上涨超过2%
                if signal['action'] == 'BUY':
                    signal['confidence'] += 0.1
                    signal['reason'].append(f"近期上涨 {recent_change:.1f}%")
            elif recent_change < -2:  # 近期下跌超过2%
                if signal['action'] == 'SELL':
                    signal['confidence'] += 0.1
                    signal['reason'].append(f"近期下跌 {recent_change:.1f}%")
    ...
    def run(self):
        ...
        for code in self.config['watchlist']:
            ...
            if signal:
                ...
                macd, signal_line, histogram = analysis['macd']
                print(f"   MACD: {macd:.4f}, 信号线: {signal_line:.4f}, 柱状图: {histogram:.4f}")
